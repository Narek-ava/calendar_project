...
function handleScroll(e) {
	if (e) {
		...
		if(bottom) {
			if ($('.cb-column').children().length % 50 == 0) {
				loadConversations($('.cb-column').children().length / 50 + 1, '', true, true)
			}
		}
	}
}
...
function searchConversations(e) {
	...
	if ($('.section-search-form').length) {
		var formData = $('.section-search-form').serializeArray();
		$.each(formData, function(index, field){
			if (field.name.indexOf('f[') === 0) {
				serializedData[field.name.slice(2, -1)] = field.value;
			}
		});
	}
	fsAjax({
		...
		q: searchQuery,
		f: serializedData,
		folder_id: isGlobalMailbox() ? getGlobalMailboxFolderId() : getGlobalAttr('folder_id'),
		mailbox_id: getGlobalAttr('mailbox_id') || urlParams.get('mailbox_id'),
		customer_id: getGlobalAttr('customer_id'),
		checkSearchPage: !!$('.input-group-lg1').length,
	...
	function(response) {
		...
		if (response.page_type == 'conversation') {
			$('.cb-column').css('display', '');
			$('#conv-layout').css('display', '');
			$('.empty-content').remove();
			$('.cb-column').empty()
			$('.cb-column').append(response.html)
		} else if (response.page_type == 'inSearchPage') {
			$('.search-results').replaceWith($(response.html).find('.search-results'))
			$('.input-group-lg1 input').val(searchQuery)
			changeMode()
		} else if (response.page_type == 'inCustomerPage') {
			$('.table-conversations, .empty-content').remove();
			$('.content-2col').append($(response.html).find('.empty-content, .table-conversations'));
		} else {
			var content2col = $(response.html).find('.content-2col');
			$('.content-2col').replaceWith(content2col);
		}
...
$(document).ready(function(){
	...
	$(document).on('click', '.recipient-email', function () {

		if ($('.calendarIframe').is(':visible')) collapsePanel($('.calendarIframe'));
		collapsePanel($('.iframeblock'))
	});
	
	$('.calendar').click( function() {

		if ($('.iframeblock').is(':visible')) collapsePanel($('.iframeblock'));
		collapsePanel($('.calendarIframe'))
	})

	$(document).on('click', '.menu-left', function () {
		var $parentDoc = $(parent.document);
		if ($(window).width() < 1100 && $(this).closest('#customerInfo').length) {
			$parentDoc.find('#conv-layout-header, #conv-layout-main').removeAttr('style');
			$parentDoc.find('#conv-layout-customer').hide()
			$parentDoc.find('.sidebar-2col').css('display','');
		} else if (!$(this).closest('#customerInfo').length) {
			let conversationId = $parentDoc.find('body:first').attr('data-conversation_id')
			$parentDoc.find('.iframeblock').attr('src', '/customer-profile?conversation_id=' + conversationId)
		}
	})

  	function collapsePanel(iframe) {
		$('.calendarIframe').css('display', 'none')
		$('.iframeblock').css('display', 'none')

		$('#conv-layout-customer').toggle()
		if ($(window).width() < 1100) {
			$('.sidebar-2col, #conv-layout-header, #conv-layout-main').css('display','none');
			$('#conv-layout-customer').css('position', 'fixed');
			iframe.contents().find('.menu-left').show()
		}
		if ($('#conv-layout-customer').is(':visible')) {
			$('#conv-layout-header, #conv-layout-main').css({
				'float': 'left',
				'padding-right': '280px',
			});
			iframe.css('display', '')
		} else {
			$('#conv-layout-header, #conv-layout-main').removeAttr('style');
		}
	}

	$(document).on('click', '.cb-row', function () {
		setGlobalAttr('conversation_id', $(this).data('conversation-id'))
		polycastInit();
		if ($(window).width() <=768) {
			$('.cb-column').css('display','none');
		}
		if ($.active == 0) {
			...
			fsAjax(payload,laroute.route('conversations.ajax'),function(response){
				...
				setGlobalAttr('conversation_id', conversation.data('conversation-id'));
...
function changeMode() {
	...
	// Set Conversations Mode query param To URL
	urlParams.set('mode', 'conversations')
	const convUrl = window.location.origin + window.location.pathname + '?' + urlParams.toString();
	$('.conv-mode-url').attr('href', convUrl)

	// Set Customers Mode query param To URL
	urlParams.set('mode', 'customers')
	const custUrl = window.location.origin + window.location.pathname + '?' + urlParams.toString();
	$('.cust-mode-url').attr('href', custUrl)
}
...
function addStylesheet() {
	...
	$(document).ready(function(){
		...
		for (var i = 0; i < cssFiles.length; i++) {
			...
			if (cssFile.href.match(/\/builds\/[a-z0-9]+\.css$/)) {
		...
		var jsFiles = document.querySelectorAll('script');
		for (var i = 0; i < jsFiles.length; i++) {
			var jsFile = jsFiles[i];
			if (jsFile.src.match(/\/builds\/[a-z0-9]+\.js$/)) {
...
function changeCustomer()
{
	if (window.parent && window.parent !== window) {
		window.parent.$('#changeCustomer').click()
	}
}
...
function switchToNewEmailConversation(type_email)
{
	...
	$('#twilio-conv-switch').removeClass('active');
	$('.email-conv-fields').show();
	$('.twilio-conv-fields').hide();
	$('#field-to').show();
	$('.sub-conv-fields').show();
	$('#name').addClass('parsley-exclude');
	$('#to').removeClass('parsley-exclude');
	$('#subject').removeClass('parsley-exclude');
	$('.dropdown-saved-replies').removeClass('dropdown-menu-left')
	$('.dropdown-saved-replies').addClass('dropdown-menu-right')
	$('.panel-heading .note-btn-group:first').find('button').css('display', '')
...
function switchToNewTwilioConversation(type_chat)
{
	$('#twilio-conv-switch').addClass('active');
    $('#email-conv-switch').removeClass('active');
	$('.twilio-conv-fields').show();
	$('.email-conv-fields').hide();
	$('#field-to').hide();
	$('.sub-conv-fields').hide();
	$('#to').addClass('parsley-exclude');
	$('#subject').addClass('parsley-exclude');
	$('#name').addClass('parsley-exclude');
	$('.panel-heading .note-btn-group:first').find('button:not(:first):not(:last)').css('display', 'none').eq(8).css('display', 'block');
	$('.conv-block:first').removeClass('conv-note-block').removeClass('conv-phone-block').addClass('conv-type-chat');
	$('.dropdown-saved-replies').removeClass('dropdown-menu-right')
	$('.dropdown-saved-replies').addClass('dropdown-menu-left')
	$('#form-create :input[name="is_note"]:first').val(0);
	$('#form-create :input[name="is_phone"]:first').val(1);
	$('#form-create :input[name="type"]:first').val(type_chat);
...
function isGlobalMailbox()
{
	return window.location.href.includes("global-mailbox");
}

function getGlobalMailboxFolderId() 
{
...
function loadConversations(page, table, no_loader, dynamicLoad = false)
{
	var filter = null;
	var params = {};
	var sorting = {};
	...
	if (getGlobalAttr('customer_id')) {
		filter['mailbox_id'] = getQueryParam('mailbox_id')
		filter['type'] = getQueryParam('type')
		filter['search'] = getQueryParam('q')
	}
	fsAjax(
		{
...
{
	if (window.parent && window.parent !== window) {
		return window.parent.$('meta[name="csrf-token"]:first').attr('content');
	}
	return $('meta[name="csrf-token"]:first').attr('content');
}

function addConvToMassagesList(conversation_id)
{
	fsAjax(
		{
			action: 'get_conversation',
			newConversationId: conversation_id,
			conversationId: getGlobalAttr('conversation_id') || conversation_id,
			folder_id: isGlobalMailbox() ? getGlobalMailboxFolderId() : $('#folders').children('li.active:first').attr('data-folder_id'),
			dynamicLoad: true,
			onlyOne: true,
			isGlobalMailbox: isGlobalMailbox()
		},
		...
		function(response) {
			...
			if (!$('.cb-row').filter('[data-conversation-id="'+ conversation_id +'"]').length) {
				if (!$('.cb-column').length) {
					const head = $('head')[0];
					const link = document.createElement('link');
					link.rel = 'stylesheet';
					link.href = '/js/summernote/summernote.css';
					head.appendChild(link);
					...
					let messageList = response.html
					const payload = {
						action:'conversation_view',
						folder_id: $('#folders').children('li.active:first').attr('data-folder_id'),
						conversation_id:conversation_id,
						global_mailbox_folder_id: $('#folders').children('li.active:first').attr('data-folder_id'),
						isGlobalMailbox: isGlobalMailbox()
					};
					fsAjax(payload,laroute.route('conversations.ajax'),function(response){
						...
						showAjaxResult(response);
						$('.empty-content').remove()
						$('.content-2col').append('<div class="cb-column">' + messageList + '</div>'+response.conversation_html)
						$('.content-2col').css('display', 'flex')
						if ($(window).width() > 768) {
							$('.content-2col').addClass('active');
						}
						if (!isGlobalMailbox()) {
					...
					$('.cb-column').prepend(response.html)
				}
			}
		}, true
	);
...
function polycastInit()
{
    ...
    if (conversation_id) {
	    ...
	    channel.on('App\\Events\\RealtimeConvNewThread', function(data, event){
			...
			if (typeof(data.thread_html) != "undefined" && data.thread_html && $('#thread-'+data.thread_id).hasClass('thread-type-draft')) {
    ...
    if (mailbox_id && el_folders.length) {
		if (isGlobalMailbox()) {
			const mailboxesList = $('.mailboxes-list');
			const targetedAs = mailboxesList.find('a[href*="/mailbox/"]');
			const numbers = [];
		
			targetedAs.each(function() {
				const href = $(this).attr('href');
				const match = href.match(/\/mailbox\/(\d+)$/);
				if (match) {
			...
			for (let i = 0; i < numbers.length; i++) {
				const channel = poly.subscribe('mailbox.' + numbers[i]);
				channel.on('App\\Events\\RealtimeMailboxNewThread', (data, event) => {
					processMailboxEvent(data, el_folders);
				});
			}
		} else {
			const channel = poly.subscribe('mailbox.' + mailbox_id);
			channel.on('App\\Events\\RealtimeMailboxNewThread', (data, event) => {
...
function processMailboxEvent(data, el_folders) {
    if (!data || typeof(data.mailbox_id) === "undefined") {
        return;
    }
    ...
    if (!$('.cb-row').filter('[data-conversation-id="'+ data.conversation_id +'"]').length) {
        addConvToMassagesList(data.conversation_id);
    }
}
...
function takeCalendarOut()
{
	if ($(window).width() >= 768) {
		// Move to the menu
		var container = $('.navbar-header .calendar:first');
		if (container) {
			container.prependTo(".navbar-right:first");
		}
	} else {
		// Move to the header
		var container = $('.navbar-right .calendar:first');
		if (container) {
			$(".navbar-header:first").prepend(container)
		}
	}
}
...
$(document).ready(function() {
	$('.collapse-btn').click(function(e) {
	  e.preventDefault();
	  $(this).parents('.panel-default').find('.panel-collapse').toggleClass('in collapse')
	});
});

if (!$(parent.document).find('.iframeblock').is(':visible')) {
	$('nav').css('display', '')
} else {
	$('.menu-left').css('display', 'inline')
	$('.layout-2col').css('cssText', 'height: 100vh !important')
	if ($(window).width() < 1100) {
		$(parent.document).find('#conv-layout-customer').css('position', 'fixed');
	}
}