...
<head>
    ...
    <title>@if ($__env->yieldContent('title_full'))@yield('title_full') @elseif ($__env->yieldContent('title'))@yield('title') - {{ config('app.name', 'Chilled Butter Inbox') }} @else{{ config('app.name', 'Chilled Butter Inbox') }}@endif</title>
    ...
    @php
        try {
            $styles= array('/css/fonts.css', '/css/bootstrap.css', '/css/select2/select2.min.css', '/js/featherlight/featherlight.min.css', '/js/featherlight/featherlight.gallery.min.css', '/css/magic-check.css', '/css/style.css', '/css/cb.css' );
                    ...
                    <div class="navbar-header">
                        ...
                        @else
                            <a class="navbar-brand" href="{{ route('dashboard') }}" title="{{ __('Dashboard') }}">
                                <img src="@filter('layout.header_logo', asset('img/logo-icon-150.png'))" height="100%" alt="" />
                    ...
                    <div class="collapse navbar-collapse" id="app-navbar-collapse">
                        ...
                        <ul class="nav navbar-nav">
                            ...
                            @elseif (count($mailboxes) > 1)
                                <li class="dropdown {{ \App\Misc\Helper::menuSelectedHtml('mailbox') }}">
                                    ...
                                    <ul class="dropdown-menu dm-scrollable mailboxes-list">
                        ...
                        <ul class="nav navbar-nav navbar-right">
                            ...
                            @guest
                                &nbsp;
                            @else
                                <li class="dropdown calendar">
                                    <a href="#" class="dropdown-toggle dropdown-toggle-icon" data-toggle="dropdown" role="button" aria-expanded="false" aria-haspopup="true" v-pre title="{{ __('Calendar') }}">
...
{{--                                        </li>--}}
                                            ...
                                            <form class="form-inline form-nav-search" role="form" action="{{ route('conversations.search') }}">
                                                <div class="input-group">
                                                    ...
                                                    <span class="input-group-btn">
                                                        <button id="searchButton" class="btn btn-default" onClick="searchConversations(event)" type="submit">{{ __('Search') }}</button>
                                                    </span>
                                                </div>
                                            </form>
                                        </li>
    ...
    {{-- Scripts --}}
    @php
        if (!\Storage::exists('js/vars.js')) \Artisan::call('freescout:generate-vars');
        try {
    @endphp
