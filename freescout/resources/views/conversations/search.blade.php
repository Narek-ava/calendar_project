...
@section('content')
	...
	<div class="section-heading section-search">
		<form action="{{ route('conversations.search') }}" class="section-search-form">
	...
	<div class="search-results">
		<ul class="nav nav-tabs nav-tabs-main margin-top">
			@if (Eventy::filter('search.is_tab_visible', true, App\Conversation::SEARCH_MODE_CONV))
				<li @if ($mode == App\Conversation::SEARCH_MODE_CONV)class="active search-tab-conv"@endif><a href="" class="conv-mode-url">{{ __('Conversations') }} <b>({{ $conversations->total() }})</b>@action('search.conversations_tab_append', $filters, $conversations->total())</a></li>
		    @endif
		    <li @if ($mode == App\Conversation::SEARCH_MODE_CUSTOMERS)class="active"@endif><a href="" class="cust-mode-url">{{ __('Customers') }} <b>({{ $customers->total() }})</b></a></li>
		</ul>
		@if ($mode == App\Conversation::SEARCH_MODE_CONV)
