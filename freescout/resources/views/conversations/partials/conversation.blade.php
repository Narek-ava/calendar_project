<div id="conv-layout" class="conv-type-{{ strtolower($conversation->getTypeName()) }}">
    <div id="conv-layout-header">
        <div id="conv-toolbar">
            ...
            <div class="conv-actions">
                <span id="back-btn" class="conv-action glyphicon glyphicon-arrow-left" data-toggle="tooltip" data-placement="bottom" title="{{ __("Back") }}" aria-label="{{ __("Back") }}" role="button"></span>
                {{-- There should be no spaced between buttons --}}
                @if (!$conversation->isPhone() && $mailbox->name !== 'Global Mailbox')
            ...
            <div class="conv-subj-block">
                <div class="conv-subjwrap">
                    <div class="conv-subjtext">
                        <div>
                            @if ($conversation->isChat() &&  $customer->first_name)
                                SMS thread with {{ $customer->first_name}}
                            @else
    ...
    <div id="conv-layout-customer">
        @if (!$conversation->isChat())
            <li role="presentation"  style="display: none"><a href="{{ route('conversations.ajax_html', ['action' =>
                            'change_customer']) }}?conversation_id={{ $conversation->id }}" data-trigger="modal" data-modal-title="{{ __("Change Customer") }}" data-modal-no-footer="true" data-modal-on-show="changeCustomerInit" tabindex="-1" role="menuitem" id="changeCustomer">{{ __("Change Customer") }}</a></li>
        @endif
        @if ($customer)
            <iframe src="{{ url('/customer-profile?conversation_id=' . $conversation->id) }}" class="iframeblock" onload="addStylesheet()"></iframe>
        @endif
        <iframe src="https://app.chilledbutter.com" class="calendarIframe"></iframe>
...
<div id="jsContent" data-js-code="@action('javascript')"></div>