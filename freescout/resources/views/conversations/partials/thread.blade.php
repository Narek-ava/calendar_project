            ...
            <div class="thread-header">
                <div class="thread-title">
                    <div class="thread-person">
                        <strong>
                            @if ($thread->type == App\Thread::TYPE_CUSTOMER)
                                @if ($thread->customer_cached)
                                    <a class="recipient-email"> {{ $thread->customer_cached->getFullName(true) }}</a>
                                @endif
                            @else
                                @if (\Helper::isPrint())
                        ...
                        @if ($thread->getFromEmail() || $thread->from)
                                <span class="from-email">
                                    {{ $thread->type == App\Thread::TYPE_CUSTOMER ? '<' . $thread->from . '>' : '<' . $thread->getFromEmail() . '>'}}
                                </span> 
                        @endif
                        ...
                        <div class="thread-recipients">
                            ...
                            @if (($thread->isForward()
                                ...
                                <div>
                                    ...
                                    @if ($thread->type !== App\Thread::TYPE_CUSTOMER)
                                        <a class="recipient-email">{{ implode(', ', $thread->getToArray()) }}</a>
                                    @else
                                        <span class="user-recipient-email">{{ implode(', ', $thread->getToArray()) }}</span>
                                    @endif
                                </div>
                            @endif
                            @if ($conversation->isChat() && $thread->type == App\Thread::TYPE_MESSAGE && $customer->first_name)
                                <strong>
                                    {{ __("To") }}:
                                </strong>
                                {{$customer->first_name}}
                ...
                <div class="thread-info">
                    ...
                    @endif
                    @if (!\Helper::isPrint())
                        <a href="#thread-{{ $thread->id }}" class="thread-date">{{ App\User::dateFormat($thread->created_at, 'M j, Y H:i A') }}</a><br/>
                    @else
