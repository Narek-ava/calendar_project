<div class="customer-snippet">
	<div class="menu-left">
		<i class="glyphicon glyphicon-menu-left" style="color:#93a1af;"></i>
	</div>
    ...
    <div class="customer-data">
		...
		<ul class="customer-contacts customer-section">
			...
			@foreach ($customer->getPhones() as $phone)
	            <li class="customer-phone">
					<i class="glyphicon glyphicon-phone "></i>
					<a href="tel:{{ $phone['value'] }}" title="{{ __('Click to Call') }}" class="contact-main">{{ $phone['value'] }}
                        @if (!\App\Customer::isDefaultPhoneType($phone['type']) || $customer->getPhones() > 1)<small class="text-muted">({{ \App\Customer::getPhoneTypeName($phone['type']) }})</small>@endif
					</a>
				</li>
	        @endforeach
		</ul>
