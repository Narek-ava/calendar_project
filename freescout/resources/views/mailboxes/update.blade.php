            ...
            <div class="col-xs-12">
                <form class="form-horizontal margin-top" method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                        ...
                        <div class="col-sm-6">
                            @if (Auth::user()->isAdmin())
                            <input id="email" type="email" class="form-control input-sized"
                                   name="email"
                                   value="{{ old('email', $mailbox->email) }}"
                                   maxlength="128" required autofocus
                                   pattern="{{ config('services.mailgun.mailbox_regex') }}"
                            >
                            @else
...
{{--                                        <option value="{{ App\Mailbox::FROM_NAME_USER }}" @if (old('from_name', $mailbox->from_name) == App\Mailbox::FROM_NAME_USER)selected="selected"@endif>{{ __("User's Name") }}</option>--}}
                        ...
                        @if (auth()->user()->isAdmin() && !!count($outboundDomains))
                            <div
                                class="form-group{{ $errors->has('from_address_prefix') || $errors->has('from_address_suffix') ? ' has-error' : '' }}">
                                <label class="col-sm-2 control-label">{{ __('Custom Outbound Address') }}</label>

                                <div class="col-sm-6">
                                    <div class="flexy">
                                        <input type="text"
                                               name="from_address_prefix"
                                               value="{{ old('from_address_prefix', $mailbox->from_address_prefix) }}"
                                               class="form-control input-sized"
                                               maxlength="255"
                                        />
                                        <div style="margin: 2px 2px 0 2px">@</div>
                                        <select name="from_address_suffix" class="form-control input-sized">
                                            @foreach($outboundDomains as $domain)
                                                <option
                                                    @if (old('from_address_suffix', $mailbox->from_address_suffix) === $domain)selected="selected"@endif
                                                >{{ $domain }}</option>
                                            @endforeach
                                        </select>
                                    </div>

                                    @include('partials/field_error', ['field'=>'from_address_prefix'])
                                    @include('partials/field_error', ['field'=>'from_address_suffix'])
                                </div>
                            </div>

                            <div class="form-group stripSubdomainBlock">
                                <label for="strip-subdomain" class="col-sm-2 control-label">{{ __('Strip Subdomain') }}</label>
                                <div class="col-sm-6">
                                    <div class="flexy">
                                        <input id="strip-subdomain" type="checkbox" name="strip_subdomain" @if ($mailbox->strip_subdomain) checked="checked" @endif>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="form-group{{ $errors->has('organization') ? ' has-error' : '' }}">
                            <label for="organization" class="col-sm-2 control-label">{{ __('Organization') }}</label>

                            <div class="col-sm-6">
                                <select id="organization" class="form-control input-sized" name="organization" autofocus>
                                    @foreach ($organizations as $organization)
                                        <option value="{{ $organization["id"] }}" @if ($organization["id"] == $mailbox->organization) selected="selected" @endif>{{ $organization["name"] }}</option>
                                    @endforeach
                                </select>

                                @include('partials/field_error', ['field'=>'organization'])
                            </div>
                        </div>
