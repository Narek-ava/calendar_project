...
class ReplyToCustomer extends Mailable
{
    ...
    public function build()
    {
        ...
        $message = $this
                    ->from($this->mailbox->getMailFromAddress(), $this->mailbox->getMailFromName())
                    ->replyTo($this->mailbox->getMailFromAddress(), $this->mailbox->getMailFromName())
                    ->subject($subject)
        ...
        if ($this->threads->first()->has_attachments) {
            foreach ($this->threads->first()->attachments as $attachment) {
                $message->attachData($attachment->getFileContents(), $attachment->file_name);
            }
        }
