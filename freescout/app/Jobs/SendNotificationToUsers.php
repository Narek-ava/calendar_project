...
class SendNotificationToUsers implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        \App\Misc\MailHelper::setMailDriver($mailbox, null, $this->conversation);
        ...
        $prev_message_id = \App\Misc\MailHelper::MESSAGE_ID_PREFIX_NOTIFICATION_IN_REPLY.'-'.$this->conversation->id.'-'.md5($this->conversation->id).'@'.$mailbox->getEmailDomain();
        ...
        foreach ($this->users as $user) {
            ...
            $message_id = \App\Misc\MailHelper::MESSAGE_ID_PREFIX_NOTIFICATION.'-'.$last_thread->id.'-'.$user->id.'-'.time().'@'.$mailbox->getEmailDomain();
