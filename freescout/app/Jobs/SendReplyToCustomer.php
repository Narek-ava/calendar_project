...
class SendReplyToCustomer implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        \App\Misc\MailHelper::setMailDriver($mailbox, $this->last_thread->created_by_user, $this->conversation);
        ...
        $this->message_id = \App\Misc\MailHelper::MESSAGE_ID_PREFIX_REPLY_TO_CUSTOMER.'-'.$this->last_thread->id.'-'.md5($this->last_thread->id).'@'.$mailbox->getEmailDomain();
        ...
        $cc_array = \App\Misc\MailHelper::removeEmailFromArray($cc_array, $this->customer_email);
        $bcc_array = \App\Misc\MailHelper::removeEmailFromArray($bcc_array, $this->customer_email);
        ...
        foreach ($cc_array as $cc_email) {
            $bcc_array = \App\Misc\MailHelper::removeEmailFromArray($bcc_array, $cc_email);
        }
