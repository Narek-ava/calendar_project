...
//            \Config::set('mail.driver', $mailbox->getMailDriverName());
            ...
            // Send emails from custom domain of mailbox
            if ($mailbox->from_address_prefix && $mailbox->from_address_suffix) {
                if ($domain = DB::table('mailgun_domains')->where('domain', $mailbox->from_address_suffix)->first()) {
                    \Config::set('services.mailgun.domain', $domain->domain);
                    \Config::set('services.mailgun.secret', $domain->secret);
                }
            }
//
//            // SMTP
...
//            \Config::set('mail.driver', \Config::get('mail.driver'));
            \Config::set('mail.from', ['address' => self::getSystemMailFrom(), 'name' => config('app.name')]);
        }
...
//        }
    ...
    public static function getSystemMailFrom()
    {
        ...
        if (!$mail_from) {
            $mail_from = 'inbox@'.\Helper::getDomain();
        }
