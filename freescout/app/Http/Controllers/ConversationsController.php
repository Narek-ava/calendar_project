...
use Modules\GlobalMailbox\Http\Controllers\GlobalMailboxController;
...
class ConversationsController extends Controller
{
    ...
    /**
     * View customer profile iframe.
     */
    public function customerProfileIframe(Request $request)
    {
        $template = 'iframes/customer_profile';
        $conversation = Conversation::findOrFail($request->query('conversation_id'));
        ...
        if ($customer) {
            ...
            $recent_conversations = $mailbox->conversations()
                ->where('customer_id', $customer->id)
                ->where('id', '<>', $conversation->id)
                ->where('status', '!=', Conversation::STATUS_SPAM)
                ->where('state', Conversation::STATE_PUBLISHED)
            ...
            $postData = [];
            if ($email = $customer->emails()->value('email')) {
                $postData = array_merge($postData, ['email' => $email]);
            }
            if ($phone = json_decode($customer->phones)) {
                $postData = array_merge($postData, ['phone' => preg_replace('/^\+1|[^0-9]+/', '', $phone[0]->value)]);
            }
            if($postData){
                try {
                    $curl = curl_init('api/inbox/companies/' . $mailbox->organization . '/search-customer');
                    curl_setopt($curl, CURLOPT_CUSTOMREQUEST, 'POST');
                    curl_setopt($curl, CURLOPT_HTTPHEADER, array(
                        "Authorization: Bearer " . env('INBOX_TO_CB_API_KEY'),
                    ...
                    foreach ($result['upcomingAppointments'] as &$appointment) {
                        $start_at = Carbon::parse($appointment['start_at'])->setTimezone(auth()->user()->timezone);
                        $appointment['start_at'] = auth()->user()->time_format === User::TIME_FORMAT_12 
                            ? $start_at->format('m/d/Y g:i a') 
                            : $start_at->format('m/d/Y H:i');
                        }
                } catch (\Throwable $th) {
                    $result = [];
                }
    ...
    public function view(Request $request, $id)
    {
        ...
        // START
        // Add conversations for simple table
        ...
        if ($request->ajax()) $template = 'conversations/partials/conversation';
        // END
    ...
    public function ajax(Request $request)
    {
        ...
        switch ($request->action) {
            ...
            case 'conversation_view':
                ...
                if (filter_var($request->input('isGlobalMailbox'), FILTER_VALIDATE_BOOLEAN)) {
                    $response['conversation_html'] = GlobalMailboxController::view($request->global_mailbox_folder_id, $conversation, true)->render();
                    session()->put('conversation_id', $conversation->id);
                    session()->put('global_mailbox_folder_id', $request->global_mailbox_folder_id);
                } else {
                    $response['conversation_html'] = $this->view($request, $conversation->id)->render();
                }
                $response['conversation_url'] = $conversation->url();
                $response['conversation_title_full'] = $conversation->getFullTitle();
                break;
            case 'dynamic_search':
                if ($request->input('customer_id')) {
                    $response['status'] = 'success';
                    $response['page_type'] = 'inCustomerPage';
                    $response['html'] = $this->conversations();
                    break;
                }
                if (filter_var($request->input('checkSearchPage'), FILTER_VALIDATE_BOOLEAN)) {
                    $response['status'] = 'success';
                    $response['page_type'] = 'inSearchPage';
                    $response['html'] = $this->search($request);
                    break;
                }
                $user = auth()->user();

                if (filter_var($request->input('isGlobalMailbox'), FILTER_VALIDATE_BOOLEAN)) {
                    $folders_data = \GlobalMailbox::getFolders($user, $request->folder_id);
                    ...
                    $query_conversations = \GlobalMailbox::getConversationsQuery($folder, $user, $mailboxes->pluck('id'), $request->q);
                    $conversations = $folder->queryAddOrderBy($query_conversations)->paginate(Conversation::DEFAULT_LIST_SIZE);
                } else {
                    $folderData = Folder::find($request->folder_id);
                    $folder = \Eventy::filter('conversations.ajax_pagination_folder', $folderData, $request, $response, $user);
                    $query_conversations = Conversation::getQueryByFolder($folder, $user->id, $request->q);
                    $conversations = $folder->queryAddOrderBy($query_conversations)->paginate(50);
                }

                $conversation = $conversations[0] ? Conversation::find($conversations[0]['id']) : '';
                $response['status'] = 'success';
                if (in_array($folderData->type ?? $request->folder_id, [Folder::TYPE_DRAFTS, Folder::TYPE_STARRED])) {
                    if (filter_var($request->input('isGlobalMailbox'), FILTER_VALIDATE_BOOLEAN)) {
                        $mailbox = new Mailbox();
                        $mailbox->id = \GlobalMailbox::MAILBOX_ID;
                        $mailbox->name = __('Global Mailbox');
                    } elseif ($user->isAdmin()) {
                        $mailbox = Mailbox::findOrFailWithSettings($request->mailbox_id, $user->id);
                    } else {
                        $mailbox = Mailbox::findOrFail($request->mailbox_id);
                    }
                    $folders = $mailbox->getAssesibleFolders();
                    $response['html'] = view('mailboxes/view', [
                        'mailbox'       => $mailbox,
                        'folders'       => $folders,
                        'folder'        => $folder,
                        'conversations' => $conversations,
                    ])->render();
                    $response['page_type'] = 'mailbox';
                    break;
                }
                $response['html'] = view('conversations/conversations_simple_table', [
                    'conversation'  => $conversation,
                    'folder'        => $folder,
                    'conversations' => $conversations
                ])->render();
                $response['page_type'] = 'conversation';
                break;
            ...
            case 'send_reply':
                ...
                $is_phone = false;
                if ($type == Conversation::TYPE_PHONE || $type == Conversation::TYPE_CHAT) {
                    $is_phone = true;
                }
                ...
                if (!$response['msg']) {
                    ...
                    if ($new) {
                        ...
                        $conversation->subject = $type == Conversation::TYPE_CHAT ? $conversation->setPreview($request->body) : $request->subject;
                        ...
                        if ($type == Conversation::TYPE_CHAT) {
                            $conversation->channel = TwilioSmsIntegration::CHANNEL_ID;
                        }
                    } else {
                        // Reply or note
                        ...
                        if ($type == Conversation::TYPE_CHAT) {
                            $conversation->subject = $conversation->setPreview($request->body);
                            $conversation->channel = TwilioSmsIntegration::CHANNEL_ID;
                        }
                        // When switching from regular message to phone and message sent
                        // without saving a draft type need to be saved here.
                        if ($conversation->type == Conversation::TYPE_EMAIL && ($type == Conversation::TYPE_PHONE || $type == Conversation::TYPE_CHAT)) {
                            $conversation->type = $type;
                        }
                    }
                    ...
                    if (!$thread) {
                        ...
                        if (($is_forward || $is_phone) && $type != Conversation::TYPE_CHAT) {
                            $thread->type = Thread::TYPE_NOTE;
                        } else {
                            $thread->type = Thread::TYPE_MESSAGE;
                        }
                    ...
                    $flash_vars = ['%tag_start%' => '<strong>', '%tag_end%' => '</strong>', '%view_start%' => '&nbsp;<a href="'.$conversation->url().'">', '%a_end%' => '</a>&nbsp;', '%undo_start%' => '&nbsp;<a href="'.route('conversations.undo', ['thread_id' => $thread->id]).'" class="text-danger">'];
                     ...
                     if ($type == Conversation::TYPE_CHAT) {
                        $flash_type = 'success';
                        if ($show_view_link) {
                            $flash_text = __(':%tag_start%Twilio SMS Sent:%tag_end% :%view_start%View:%a_end% or :%undo_start%Undo:%a_end%', $flash_vars);
                        } else {
                            $flash_text = __(':%tag_start%Twilio SMS Sent:%tag_end% :%undo_start%Undo:%a_end%', $flash_vars);
                        }
                    } elseif ($is_phone) {
                        $flash_type = 'warning';
                        if ($show_view_link) {
            ...
            case 'save_draft':
                ...
                if (!$response['msg']) {
                    ...
                    // Thread type.
                    if ($is_create && !empty($request->is_note) && $type != Conversation::TYPE_CHAT) {
                        $thread->type = Thread::TYPE_NOTE;
                    } else {
                        $thread->type = Thread::TYPE_MESSAGE;
                    }
            ...
            case 'get_conversation':
                $folderData = Folder::find($request->folder_id);
                if (!empty($folderData) && in_array($folderData->type, [Folder::TYPE_ASSIGNED, Folder::TYPE_MINE, Folder::TYPE_UNASSIGNED])) {
                    $response = $this->ajaxConversationsPagination($request, $response, $user);
                }
                $response['status'] = 'success';
                break;
    ...
    public function ajaxConversationsPagination(Request $request, $response, $user)
    {
        ...
        if (!$response['msg']) {
            $query_conversations = Conversation::getQueryByFolder($folder, $user->id);
            if (filter_var($request->input('onlyOne'), FILTER_VALIDATE_BOOLEAN)) {
                if (filter_var($request->input('isGlobalMailbox'), FILTER_VALIDATE_BOOLEAN)) {
                    $folders_data = \GlobalMailbox::getFolders($user, $request->folder_id);
                    $folders = $folders_data['folders'];
                    $folder = $folders_data['folder'];
                    ...
                    \GlobalMailbox::$folders = $folders;
                    $query_conversations = \GlobalMailbox::getConversationsQuery($folder, $user, $mailboxes->pluck('id'), null, $request->newConversationId);
                } else {
                    $query_conversations = Conversation::getQueryByFolder($folder, $user->id, null, $request->newConversationId);
                }
            }
        ...
        if (filter_var($request->input('dynamicLoad'), FILTER_VALIDATE_BOOLEAN)) {
    ...
    public function search(Request $request)
    {
        ...
        $view = view('conversations/search', [
            'folder'        => $folder,
            'q'             => $request->q,
            'filters'       => $filters,
        ...
        return $request->action == 'dynamic_search' ? $view->render() : $view;
    }
    ...
    public function conversations()
    {
        ...
        $conversationsId = request()->input('q') ? Conversation::getConversationsIdDuringSearch(request()->input('q')) : [];
        $conversations = $customer->conversations()
            ->where('customer_id', $customer->id)
            ->when(request()->filled('mailbox_id'), function ($query) {
                return $query->where('mailbox_id', request()->input('mailbox_id'))
                    ->where('status', '!=', Conversation::STATUS_SPAM)
                    ->where('state', Conversation::STATE_PUBLISHED);
            })
            ->whereIn('mailbox_id', auth()->user()->mailboxesIdsCanView())
            ->when(request()->input('q'), function ($query) use ($conversationsId) {
                return $query->whereIn('conversations.id', $conversationsId);
            })
            ->when(request()->filled('type'), function ($query) {
                return $query->where('created_at', request()->input('type') == 'recent' ? '>=' : '<', Carbon::now()->subDays(7));
            })
            ->orderBy('created_at', 'desc')
    ...
    public function ajaxConversationsFilter(Request $request, $response, $user)
    {
        ...
        $response['html'] = view('conversations/conversations_table', [
            ...
            'conversations_filter' => isset($request->filter['customer_id']) ? $request->filter : []
        ])->render();
    ...
    {
        $conversationsId = $request->filter['search'] ? Conversation::getConversationsIdDuringSearch($request->filter['search']) : [];
        // Get IDs of mailboxes to which user has access
        $mailbox_ids = $request->filter['mailbox_id'] ? [$request->filter['mailbox_id']] : $user->mailboxesIdsCanView();
        $type = $request->filter['type'] ?? '';
        $query_conversations = Conversation::whereIn('conversations.mailbox_id', $mailbox_ids)
            ->when($type, function ($query) use ($type) {
                return $query->where('created_at', $type == Conversation::RECENT_CONVERSATION ? '>=' : '<', Carbon::now()->subDays(7));
            })
            ->when($request->filter['search'], function ($query) use ($conversationsId) {
                return $query->whereIn('conversations.id', $conversationsId);
            })
    ...
    public function processPhoneCustomer($request)
    {
        ...
        if (trim($request->name ?? '') || trim($request->phone ?? '')) {
            ...
            $customer_data['first_name'] = $request->type == Conversation::TYPE_CHAT ? $request->phone : $name_parts[0];
