...
class MailboxesController extends Controller
{
    ...
    public function createSave(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'email' => ['required','string','email','max:128','unique:mailboxes', 'regex:/' . config('services.mailgun.mailbox_regex') . '/i'],
    ...
    public function update($id)
    {
        ...
        if (auth()->user()->isAdmin()) {
            try {
                $response = (new Client())->get('https://api.mailgun.net/v4/domains', [
                    'auth' => ['api', config('services.mailgun.api_key')]
                ]);
                $mailgunDomains = json_decode($response->getBody()->getContents(), true);
                $outboundDomains = array_map(fn($domain) => Arr::get($domain, 'name'), Arr::get($mailgunDomains, 'items'));

                $domainsWithSecret = DB::table('mailgun_domains')->get()->pluck('domain')->toArray();
                $outboundDomains = array_intersect($outboundDomains, $domainsWithSecret);
            } catch (Throwable $e) {
                // Admin is allowed to see exceptions
                Session::flash('flash_error_floating', "Mailgun fetch domains error: {$e->getMessage()}");
            }
        }
        $curl = curl_init('api/inbox/companies');
        ...
        return view('mailboxes/update', [
            'mailbox'         => $mailbox,
            'mailbox_user'    => $mailbox_user,
            'organizations'   => $result,
            'flashes'         => $this->mailboxActiveWarning($mailbox),
    ...
    public function updateSave($id, Request $request)
    {
        ...
        if ($user->can('updateSettings', $mailbox)) {
            ...
            $validator = Validator::make($request->all(), [
                'name'                => 'required|string|max:40',
                'email'               => ['required', 'string', 'email', 'max:128', "unique:mailboxes,email,$id", 'regex:/' . config('services.mailgun.mailbox_regex') . '/i'],
                'aliases'             => 'nullable|string|max:255',
                'from_name'           => 'required|integer',
                'from_name_custom'    => 'nullable|string|max:128|required_if:from_name,' . Mailbox::FROM_NAME_CUSTOM,
                'ticket_status'       => 'required|integer',
                'template'            => 'required|integer',
                'ticket_assignee'     => 'required|integer',
                'organization'        => 'required|integer',
                'from_address_prefix' => ['sometimes', 'nullable', 'string', 'max:255', function ($attribute, $value, $fail) {
                    // Basic check for only prefix from emails is entered
                    if (Str::contains($value, ['@'])) $fail('The ' . $attribute . ' is invalid.');
                }],
                'from_address_suffix' => 'sometimes|nullable|string|max:255',
            ]);
        ...
        $request->merge([
            'strip_subdomain' => !!$request->input('strip_subdomain')
        ]);
...
//        return redirect()->route('mailboxes.connection.incoming', ['id' => $id]);
    ...
    public function view($id, $folder_id = null)
    {
        ...
        // Try to open the first message except Drafts or Starred folders
        if (($conversation = $conversations->first()) && !in_array($folder->type, [Folder::TYPE_DRAFTS, Folder::TYPE_STARRED])) {
            return redirect()->route('conversations.view', [$conversation, 'page' => 1]);
        }
        ...
        switch ($request->action) {
            ...
            case 'send_test':
                ...
                if (!$response['msg']) {
                    ...
                    try {
                        $test_result = \App\Misc\MailHelper::sendTestMail($request->to, $mailbox);
                    } catch (\Exception $e) {
