    ...
    public function conversations($id)
    {
        ...
        $conversations = $customer->conversations()
            ...
            ->when(request()->filled('mailbox_id'), function ($query) {
                return $query->where('mailbox_id', request()->query('mailbox_id'))
            ...
            ->when(request()->filled('type'), function ($query) {
                return $query->where('created_at', request()->query('type') == Conversation::RECENT_CONVERSATION ? '>=' : '<', Carbon::now()->subDays(7));
            })
            ...
            if ($request->show_fields != 'all') {
                switch ($request->show_fields) {
                    ...
                    case 'phone':
                        ...
                        if (!$text && !is_null($customer->phones)) {
                            if (strstr($customer->first_name, $q)) {
                                $text = $phones[0]['value'];
                                if ($customer->getFullName()) {
                                    $text .= ' — '.$customer->getFullName();
                                }
