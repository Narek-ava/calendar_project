...
class Thread extends Model
{
    ...
    public function getFromEmail($conversation_number = '')
    {
        $email = '';
        if ($this->state == self::STATE_DRAFT && !empty($this->edited_by_user_id)) {
            // Draft
            if (auth()->user() && $this->edited_by_user_id == auth()->user()->id) {
                $email = auth()->user()->email;
            } else {
                $email = $this->edited_by_user->email;
            }
        } elseif ($this->created_by_user_cached) {
            if ($this->created_by_user_id && auth()->user() && $this->created_by_user_cached->id == auth()->user()->id) {
                $email = auth()->user()->email;
            } else {
                $email = $this->created_by_user_cached->email;
            }
        }

        $email = \Eventy::filter('thread.action_person', $email, $this, $conversation_number);
