...
{%mailbox.name%}</span>';
     ...
     */
    protected $fillable = [
        'name', 'email', 'aliases', 'auto_bcc', 'from_name', 'from_name_custom', 'ticket_status',
        'ticket_assignee', 'template', 'before_reply', 'signature', 'out_method', 'out_server',
        'out_username', 'out_password', 'out_port', 'out_encryption', 'in_server', 'in_port',
        'in_username', 'in_password', 'in_protocol', 'in_encryption', 'in_validate_cert', 'auto_reply_enabled',
        'auto_reply_subject', 'auto_reply_message', 'office_hours_enabled', 'ratings', 'ratings_placement',
        'ratings_text', 'imap_sent_folder',
        'from_address_prefix', 'from_address_suffix', 'organization', 'strip_subdomain'
    ];
    ...
    {
        // Our mailboxes always are active since we do not need to configure smtp or another shit
        return true;
//        return $this->isInActive() && $this->isOutActive();
    ...
    {
        // self::FROM_NAME_USER is not work on auto-reply so get rid of this feature
        if ($this->from_name == self::FROM_NAME_CUSTOM && $this->from_name_custom) return $this->from_name_custom;
        return $this->name;
    }
    ...
    {
        if ($this->from_address_prefix && $this->from_address_suffix) {
            $from_address_suffix = $this->from_address_suffix;
            if ($this->strip_subdomain && strpos($this->from_address_suffix, '.')) {
                preg_match('/[^.]+\.[^.]+\z/', $this->from_address_suffix, $matches);
                $from_address_suffix = $matches[0];
            }
            return "$this->from_address_prefix@$from_address_suffix";
        }
        return $this->email;
    }
