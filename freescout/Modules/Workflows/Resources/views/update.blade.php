...
@section('content')
    ...
    <div class="section-heading">
        <a href="{{ route('mailboxes.workflows', ['id' => $mailbox->id]) }}" class="btn btn-link padding-left-0"><i
                class="glyphicon glyphicon-arrow-left"></i>
            <a href="{{ route('workflows.help') }}" class="small link-help pull-right"
               data-trigger="modal"
               data-modal-size="lg"
               data-modal-title="{{ __('Workflows Help') }}"
               data-modal-no-footer="true">
                <i class="glyphicon glyphicon-question-sign"></i> &nbsp;{{ __('Workflows Help') }}
            </a>
        {{ __('Workflows') }}
    </div>
    ...
    <form class="form-horizontal" method="POST" action="">
        ...
        <div
            class="form-group{{ $errors->has('name') ? ' has-error' : '' }} margin-top form-inline margin-left margin-right">
            <label for="name" class="control-label">{{ __('Workflow Name') }}</label>&nbsp;

            <input id="name" type="text" class="form-control wf-name" name="name"
                   value="{{ old('name', $workflow->name) }}" maxlength="75" required="required">

            @include('partials/field_error', ['field'=>'name'])
        </div>

        <ul class="nav nav-tabs nav-tabs-main margin-top" role="tablist">
            <li class="active"><a href="#tab-settings" aria-controls="tab-settings" role="tab"
                                  data-toggle="tab">{{ __('Settings') }}</a></li>
            <li id="wf-conditions-tab" @if ($workflow->isManual()) class="hidden" @endif><a href="#tab-conditions"
                                                                                            @if ($mode != 'create' && isset($workflow->errors()['conditions'])) class="text-danger"
                                                                                            @endif aria-controls="tab-conditions"
                                                                                            role="tab"
                                                                                            data-toggle="tab">{{ __('Conditions') }}</a>
            </li>
            <li><a href="#tab-actions"
                   @if ($mode != 'create' && isset($workflow->errors()['actions'])) class="text-danger"
                   @endif aria-controls="tab-actions" role="tab" data-toggle="tab">{{ __('Actions') }}</a></li>
        </ul>
                ...
                <div class="col-xs-12">
                    ...
                    <div class="tab-content">
                        <div role="tabpanel" class="tab-pane active" id="tab-settings">
                            ...
                            <div class="form-group{{ $errors->has('type') ? ' has-error' : '' }}">
                                ...
                                <div class="col-sm-8">
                                    <select id="type" class="form-control input-sized" name="type" required autofocus>
                                        <option value="{{ Workflow::TYPE_AUTOMATIC }}"
                                                @if (old('type', $workflow->type) == Workflow::TYPE_AUTOMATIC)selected="selected"@endif>{{ __('Automatic') }}</option>
                                        <option value="{{ Workflow::TYPE_MANUAL }}"
                                                @if (old('type', $workflow->type) == Workflow::TYPE_MANUAL)selected="selected"@endif>{{ __('Manual') }}</option>
                                    </select>
                            ...
                            <div
                                class="form-group{{ $errors->has('apply_to_prev') ? ' has-error' : '' }} wf-type wf-type-auto @if (old('type', $workflow->type) != Workflow::TYPE_AUTOMATIC) hidden @endif">
                                <label for="apply_to_prev"
                                       class="col-sm-2 control-label">{{ __('Apply to Previous') }}</label>

                                <div class="col-sm-6">
                                    <div class="controls">
                                        <div class="onoffswitch-wrap">
                                            <div class="onoffswitch">
                                                <input type="checkbox" name="apply_to_prev" value="1" id="apply_to_prev"
                                                       class="onoffswitch-checkbox"
                                                       @if (old('apply_to_prev', $workflow->apply_to_prev))checked="checked"@endif >
                                            ...
                                            <i class="glyphicon glyphicon-info-sign icon-info icon-info-inline"
                                               ...
                                               data-content="{{ __('Apply this workflow to all previous conversations matching conditions') }}"></i>
                                        </div>
                                        @if (!$workflow->apply_to_prev)
                                            <div class="hidden form-help" id="apply-to-prev-alert">
                                                <span
                                                    class="text-warning">{{ __('When this options is enabled the workflow may be applied to a large number of the past conversations and changes can not be undone. It is recommended to backup the application before turning this option on.') }}</span>
                                            </div>
                                        @endif
                                ...
                                <div class="onoffswitch-wrap">
                                    <div class="onoffswitch">
                                        <input type="checkbox" name="active" value="1" id="active"
                                               class="onoffswitch-checkbox"
                                               @if (old('active', $workflow->active))checked="checked"@endif >
                    ...
                    <div class="form-group margin-top">
                        <div class="col-sm-6 col-sm-offset-2">
                            ...
                            @if ($mode != 'create')
                                <a href="#" id="wf-delete-trigger" data-loading-text="{{ __('Deleting') }}…"
                                   data-wf_id="{{ $workflow->id }}"
                                   class="btn btn-link text-danger">{{ __('Delete') }}</a>
                            @endif
                        </div>
                    </div>
                </div>
